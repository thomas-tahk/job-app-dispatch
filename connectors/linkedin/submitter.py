...
def submit_easy_apply(page, req: SubmitRequest) -> SubmitResult:
    """
    Attempt LinkedIn Easy Apply flow.
    Falls back to manual URL if multi-step questions are too complex.
    TODO: implement.
    """
    return SubmitResult(
        success=False,
...
def submit_external(req: SubmitRequest) -> SubmitResult:
    """
    External application redirect — automation not feasible for arbitrary company sites.
    Return the URL so the user can apply manually.
    """
    return SubmitResult(
        success=False,
...
def submit(req: SubmitRequest) -> SubmitResult:
    if not req.job.is_easy_apply:
        return submit_external(req)

    with sync_playwright() as p:
        context = get_browser_context(p, headless=True)
        page = context.new_page()
        try:
            result = submit_easy_apply(page, req)
